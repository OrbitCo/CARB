...
class MapData extends React.Component<Props, State> {
  ...
  fetchAirGrantSensors = () => {
    // TEMP: don't display any monitors as air grant sensors for now...
    // will eventually organize and display monitors by 'monitorPurpose' field
  ...
  fetchRegulatorySensors = () => {
    // TEMP: display all monitors as regulatory sensors for now...
    // will eventually organize and display monitors by 'monitorPurpose' field
    fetch(`${apiUrl}/Monitors`)
      .then((res) => res.json())
      .then((data) => {
        ...
        this.plotRegulatorySensors(data);
      })
      .catch((err) => {
        console.error(err);
        this.setState({ regulatorySensors: true });
